...
import {Category, Difficulty, ApiQuestion, Question, Results, ISelectData} from '../data.models';
...
export class QuizService {
  ...
  createQuiz(categoryId: string, difficulty: Difficulty): Observable<Question[]> {
    ...
    return this.http.get<{ results: ApiQuestion[] }>(
      ...
      .pipe(
        map(res => {
          const quiz: Question[] = res.results.map((q, index) => (
